...
class Task(torch.nn.Module, metaclass=ABCMeta):
    ...
    def __restore_checkpoint(self, m: str, path: str):
        ...
        #checkpoint = torch.load(path)#works with  cuda
        ...
        saved=self.task_models['RGB'].module.base_model.logits.conv3d
        self.task_models['RGB'].module.base_model.logits.conv3d=torch.nn.Conv3d(1024,8,kernel_size=(1,1,1), stride=(1,1,1))
        self.task_models[m].load_state_dict(checkpoint["model_state_dict"], strict=False)#strict was True: everything must match
        self.task_models['RGB'].module.base_model.logits.conv3d=saved
        #self.task_models['RGB'].module.num_class=20
